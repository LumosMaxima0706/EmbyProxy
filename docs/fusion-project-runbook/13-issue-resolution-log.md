...
| ISSUE-PUBLISH-003 | BWG publish bridge attempt 2 | Push-precheck assertion expected the remote feature ref to equal the new target before push | Retry script compared remote ref to target instead of the expected base; the check was ordered before push | BWG fast-forward merge completed to `61de764`, but push was not attempted; remote stayed at `c7f475c` | No | Compare remote to base before push, then to target only after push; continue from the clean BWG ff-only head using a new bundle that adds any new local audit commit | Resolution plan recorded; BWG branch is clean at `61de764`, remote remains base, no push occurred | IN_PROGRESS | TBD |
| ISSUE-PUBLISH-001 | BWG publish bridge readiness | Remote feature branch is clear and local target is ahead, but direct `git push --dry-run` could not authenticate; BWG bridge was not used | Local environment has no usable publisher authentication, and the operating rules require BWG as the GitHub publisher | Publish gate blocked; implementation and local commits are unaffected | Yes for publish only; No for implementation | Create and locally verify a bundle for the approved target, then transfer only through an explicitly authorized BWG SSH/SCP gate; otherwise wait for human transfer | Readiness checks passed except publisher access; no actual push occurred; no remote/auth configuration was changed | BLOCKED | TBD |
