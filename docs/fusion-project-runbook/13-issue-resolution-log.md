...
| ISSUE-OWNER-ADMIN-RETRY-001 | Owner Admin basic-only guarded live verification | Public Admin page verification ended with an HTTP/2 stream error after the new release and Nginx config had applied | Verification fetched the full Admin HTML through a reused HTTP/2 client path; no app panic/fatal evidence was found, and the error trap restarted the sidecar while rolling back | The safety trap restored the prior release and configuration; owner-admin remained protected but the login-loop fix was not left deployed | Yes for live completion | Keep the verified backup/rollback, force HTTP/1.1 for the retry, write HTML to a root-only temporary file, and use fixed-string assertions | Automatic rollback passed; HTTP/1.1 temporary-file verification subsequently passed the complete public matrix | DONE | `0fc2334` |
| ISSUE-OWNER-ADMIN-RETRY-002 | Owner Admin basic-only guarded reapply preflight | Artifact version gate invoked unsupported `-version`, so the binary entered normal startup and exited because the standalone preflight process had no Admin configuration | The project supports `version`, `--version`, or `-v`; the retry script used `-version` | No live change occurred before this gate; the ERR trap restored the already-current old release/config and health checks passed | No | Change the gate to `version`, require both release label and commit, then rerun remote syntax and guarded apply | Gate corrected; release and Nginx staging proceeded on retry, then a separate listener-readiness race was identified | DONE | `0fc2334` |
| ISSUE-OWNER-ADMIN-RETRY-003 | Owner Admin basic-only guarded reapply | Immediate `ss` assertion failed directly after systemd reported the restarted sidecar active | The service became active before its loopback socket was observable; the gate had no bounded readiness wait | ERR trap restored the old release/config; post-rollback sidecar, Nginx, timer, listener, and NOSLA target passed | No | Poll only the localhost listener for up to 15 seconds, fail and roll back if it never appears, then continue scoped verification | Bounded readiness gate passed; release `0fc2334` applied with loopback listener and `NRestarts=0` | DONE | `0fc2334` |
| ISSUE-OWNER-ADMIN-RETRY-004 | Owner Admin basic-only public matrix | Unauthenticated request returned 401, but Basic-authenticated `/admin` returned 400 while an equivalent localhost trusted request returned 200 | The owner-admin location included the shared `proxy_params`, which set `Host`, and then set the exact owner-admin Host again; the duplicate Host was rejected by Go before the handler ran | Basic Auth remained enforced and backend remained loopback-only, but the public Admin UI was unavailable | Yes for owner login | Remove the shared include only from the dedicated owner-admin locations and explicitly set one Host plus the required forwarding headers; run Nginx staging syntax before scoped reload | Root-only pre-fix backup verified; staging/live Nginx syntax passed; Basic UI/auth status/Admin API now 200 and token login/media path remain 404 | DONE | `0fc2334` |
| ISSUE-OWNER-ADMIN-ISOLATION-001 | Post-fix hostname isolation matrix | Production `stream /admin` redirected to an Admin login instead of returning 404/403 | NOSLA's existing `stream-proxy-admin-locations.inc` explicitly proxied `/admin/` and `/api/admin/` to its local Admin service | Public Admin remained protected by its existing application login, but the media hostname violated the newly authorized dedicated-owner-admin-only boundary | Yes for final isolation gate | Back up only the NOSLA include, generate exact rollback, replace Admin exact/prefix locations with 404 while preserving `/s/`, run staged Nginx syntax, reload only NOSLA Nginx, and verify small endpoints | Backup/checksums/rollback syntax and isolated Nginx dry-run passed; NOSLA Nginx-only apply passed; all stream Admin exact/prefix paths return 404 and stream health remains 200 | DONE | `0fc2334` |
| ISSUE-OWNER-PUBLIC-URL-001 | Owner Admin node-card public address | UHD displayed an owner-admin `/uhd` URL, which correctly returned Nginx 404 | The frontend derived node links from `location.origin` and node name instead of a topology-owned public media URL | Display, copy, and reveal/preview produced a non-media Admin-host link | Yes for owner workflow | Add validated backend public-media configuration, return a secret-free `publicUrl`, and make all three UI actions consume only that value | Deployed; Basic-only UI/API, backend URL contract, display/copy/preview binding, isolation, small public endpoints, listener/policy/timer, and logs PASS | DONE | `f4d2d9d` |
| ISSUE-OWNER-PUBLIC-URL-002 | Public-route discovery | Neither production `/uhd` nor `/s/v1` is a real UHD route; common upstream Web UI paths also return upstream/Cloudflare 404 | UHD is a legacy node, `v1` belongs to another managed-route node, and production uses an upstream-path proxy location; upstream does not serve the common browser Web paths | A guessed `/uhd` or `/s/v1` fix would still be wrong; browser login-page verification cannot be honestly passed against the current upstream | No for client/base URL correction; external limitation for browser login | Use the verified existing production path explicitly, validate only small information endpoints automatically, and record the upstream Web UI limitation without changing upstream | Discovery and public small-interface verification complete; upstream, Nginx, and route config unchanged | DONE | `f4d2d9d` |
...
| FAILADMIN-BACKUP-001 | Backup verification | Checksum verification reported one mismatch | Initial manifest included itself while it was being written | Regenerated manifests excluding self/checksum files; both hosts verified before staging | DONE |
| FAILADMIN-METER-001 | Restricted counter setup | Direct BWG-to-NOSLA public-key SCP lost connection | BWG lacked the workstation NOSLA SSH alias/trust chain | Confirmed no account/live file, then relayed only the public key with SCP third-party mode; forced command and forwarding denial passed | DONE |
| FAILADMIN-ADMIN-001 | Admin staging | Basic-authenticated Admin returned 500 twice | First staging htpasswd was root-only unreadable; then isolated Nginx defaulted workers to `nobody`, unlike live `www-data` | Used root:www-data 0640 hash file, root-only 0600 password file, and live worker identity; full two-layer staging/live matrix passed | DONE |
| FAILADMIN-AUTO-001 | Pre-auto snapshot | First snapshot stopped on a nonexistent presumed BWG Nginx filename | Production/staging filenames differed between hosts | Confirmed mode/timers/DNS unchanged, replaced assumption with exact existing BWG allowlist, reran snapshot, then applied successfully | DONE |
| FAILADMIN-VERIFY-001 | Post-apply verification | Parallel SCP verification batch closed one connection | Concurrent transfers were unreliable; critical public health was still unknown | Immediately verified public health/SSH, then reran BWG and NOSLA scripts sequentially; all passed | DONE |
| FAILADMIN-ACME-001 | Secure Admin certificate | Owner reported HTTP 429/retry-limit signal on 2026-08-12; exact event time unavailable | ACME/provider throttling signal; not attributable from bounded local log | `blocked_by_acme_rate_limit`; `wait_and_retry_later`. Froze all production ACME retries. Current certificate independently validates through 2026-11-10 and live Admin security passes; staging-only validation required before future renewal action | BLOCKED |
| FAILADMIN-TIMER-001 | Closeout observation | New timer was active/enabled but `active (elapsed)` with no next trigger after auto apply | `OnUnitActiveSec` had no usable timer-owned monotonic anchor after the oneshot was manually run while the timer was stopped; an `OnUnitInactiveSec`-only staging attempt also lacked its first anchor | Backed up timer; combined `OnActiveSec=5min` with `OnUnitInactiveSec=5min`; verified unit syntax, finite next trigger, another completed cycle, and single-controller state | DONE |
| FAILADMIN-AUTH-002 | Owner Admin login | Basic Auth succeeded, then the embedded ADMIN_TOKEN/session flow returned the browser to authentication | Two independent interactive authentication layers had incompatible browser/session lifecycle | Backed up live state; added default-off loopback/exact-Host/header/path trusted mode, disabled token/2FA endpoints only there, hid token UI, and retained token auth everywhere else | IN PROGRESS |
| PLAYBACK-VOD1-001 | Yamby UHD playback | Library metadata and images loaded, but media playback repeatedly spun with no speed | Primary playback returned 302 to an explicitly rewritten `v1-vod1` proxy path; NOSLA lacked that path and its default location returned 403 before sidecar/upstream | Backed up both scopes, changed the display base to no trailing slash, and added only `v1-vod1` exact/prefix locations matching the existing no-cache/Range stanza | APPLIED; owner playback evidence pending |
| PLAYBACK-BACKUP-001 | Backup wrapper | First generated backup directory names contained a trailing CR from the Windows-to-SSH script wrapper | PowerShell here-string CRLF reached the remote timestamp variable | Kept those partial artifacts out of the rollback contract; created explicit fixed-path backups and reverified checksums, permissions, and `bash -n` before apply | DONE |
