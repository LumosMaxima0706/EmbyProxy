...
| FAILADMIN-003 | 3 | Establish reliable traffic accounting and quotas | IN_PROGRESS | FAILADMIN-001 | `deploy/failover/*`, `33` | Owner provider seeds plus restricted host RX+TX estimate implemented and tested locally; remote meter pending | Create verified backups, then install restricted meter |
| FAILADMIN-004 | 5-9 | Backup, select apply adapter, canary, production apply, secure Admin | PENDING | FAILADMIN-003 | deployment/runbook files | NOT RUN; templates implemented locally only | Backup and remote dry-run before any controller/DNS/Admin mutation |
