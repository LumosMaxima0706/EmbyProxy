...
SELECT uid, node_name, route_slug, public_url, status, reason, failed_step,
       nosla_status, bwg_status, playback_status, playback_verified_at, updated_at
FROM emby_publications WHERE uid = ? AND node_name = ?
`, uid, nodeName).Scan(
		&p.UID, &p.NodeName, &p.RouteSlug, &p.PublicURL, &p.Status, &p.Reason,
		&p.FailedStep, &p.NOSLAStatus, &p.BWGStatus, &p.PlaybackStatus, &p.PlaybackVerifiedAt, &p.UpdatedAt,
	)
...
SET playback_status = 'ok', playback_verified_at = ?, updated_at = MAX(updated_at, ?)
